...
function session_info = pid_startup_gui
    WINDOW_WIDTH = 500;
    WINDOW_HEIGHT = 300;
    ...
    function selected =  get_session_type(app, event)
        selected = ButtonGroup.SelectedObject.Text;
    end
    ...
    function submit_callback(app, event)
        ...
        valid = validate_input(name, odor, x1, x5, x10);

        if valid == false
            msgbox("Please correct the input in the red fields!", 'Error', "error");
        else
            session_info.name = name;
            session_info.odor = odor;
            session_info.x1 = x1;
            session_info.x5 = x5;
            session_info.x10 = x10;
            session_info.session_type = get_session_type();
            %assignin('base', 'session_info', session_info);
            uiresume(gui);
            close(gui);
        end
    end
