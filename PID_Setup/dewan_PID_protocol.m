...
function get_analog_data(a_in, BpodSystem)
    ...
    if num_bytes_to_read > num_bytes_per_frame % Are there at least 4 bytes (1 frame) available to read?
        ...
        if data_prefix == 'R' || data_prefix == '#' % R is raw data, # is sync events
            ...
            %% Byte 3 & 4
            data_samples = typecast(data(1:end), 'uint16'); % Cast all the data to 16-bit ints (2 byte message -> 1 value)
...
function resolution = get_voltage_resolution(a_in)
    ...
    resolution = total_range / 2^32; % Divide total range by bit resolution of ADC (32-bit) should give V/bit (very small number)
